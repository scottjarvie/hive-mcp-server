...
server.tool(
  ...
  "Create a new blog post on the Hive blockchain using the configured account credentials.",
  {
    ...
    body: z.string().min(1).describe("Content of the blog post, can include Markdown formatting"),
    tags: z.union([
      z.array(z.string()),
      z.string().transform((val, ctx) => {
        // Handle empty string
    ...
    ]).default(['blog']).describe("Tags for the post. Can be provided as comma-separated string 'blog,life,writing' or array"),
    beneficiaries: z.union([
      ...
      z.null()
    ]).optional().nullable().describe("Optional list of beneficiaries to receive a portion of the rewards"),
    permalink: z.string().optional().describe("Optional custom permalink. If not provided, one will be generated from the title"),
    max_accepted_payout: z.string().optional().describe("Optional maximum accepted payout (e.g. '1000.000 HBD')"),
    percent_hbd: z.number().min(0).max(10000).optional().describe("Optional percent of HBD in rewards (0-10000, where 10000 = 100%)"),
    allow_votes: z.boolean().optional().default(true).describe("Whether to allow votes on the post"),
    allow_curation_rewards: z.boolean().optional().default(true).describe("Whether to allow curation rewards")
  },
  async ({ title, body, tags, beneficiaries, permalink, max_accepted_payout, percent_hbd, allow_votes, allow_curation_rewards }) => {
    try {
      // Get credentials from environment variables
      const username = process.env.HIVE_USERNAME;
      ...
      const finalPermalink = permalink || title
        .toLowerCase()
...
server.tool(
  ...
  {
    parent_author: z.string().describe("Username of the post author or comment you're replying to"),
    parent_permlink: z.string().describe("Permlink of the post or comment you're replying to"),
    body: z.string().min(1).describe("Content of the comment, can include Markdown formatting"),
    permalink: z.string().optional().describe("Optional custom permalink for your comment. If not provided, one will be generated"),
    beneficiaries: z.union([
      z.array(z.object({
        account: z.string(),
        weight: z.number().min(1).max(10000)
    ...
    max_accepted_payout: z.string().optional().describe("Optional maximum accepted payout (e.g. '1000.000 HBD')"),
    percent_hbd: z.number().min(0).max(10000).optional().describe("Optional percent of HBD in rewards (0-10000, where 10000 = 100%)"),
    allow_votes: z.boolean().optional().default(true).describe("Whether to allow votes on the comment"),
    allow_curation_rewards: z.boolean().optional().default(true).describe("Whether to allow curation rewards")
  },
  async ({ parent_author, parent_permlink, body, permalink, beneficiaries, max_accepted_payout, percent_hbd, allow_votes, allow_curation_rewards }) => {
    try {
      // Get credentials from environment variables
      const username = process.env.HIVE_USERNAME;
      const postingKey = process.env.HIVE_POSTING_KEY;
      ...
      const finalPermalink = permalink || `re-${parent_permlink.slice(0, 20)}-${Date.now().toString(36)}`;
...
server.tool(
  ...
  "Sign a message using a Hive private key from environment variables.",
  {
    ...
    key_type: z.enum(["posting", "active", "memo"]).optional().default("posting").describe("Type of key to use: 'posting', 'active', or 'memo'. Defaults to 'posting' if not specified.")
  },
    ...
    try {
      ...
      try {
        ...
        return {
          content: [{ 
            type: "text" as const, 
            text: `Error: Invalid ${key_type} key format: ${error instanceof Error ? error.message : String(error)}`
          }],
          isError: true
        };
      }
...
server.tool(
  ...
  "Verify a message signature against a Hive public key",
  {
    ...
    public_key: z.string().describe("Public key to verify against (with or without the STM prefix)")
  },
    ...
    try {
      ...
      try {
        ...
        return {
          content: [{ 
            type: "text" as const, 
            text: `Error: Invalid public key format: ${error instanceof Error ? error.message : String(error)}` 
          }],
          isError: true
        };
      }
      ...
      try {
        ...
        return {
          content: [{ 
            type: "text" as const, 
            text: `Error: Invalid signature format: ${error instanceof Error ? error.message : String(error)}` 
          }],
          isError: true
        };
      }
...
const startServer = async () => {
  ...
  if (!process.env.HIVE_ACTIVE_KEY) {
    logger("Warning: HIVE_ACTIVE_KEY environment variable is not set (required for token transfers)");
  } else {
